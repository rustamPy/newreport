...
class ReportGenerator:
    ...
    def _get_image_as_base64(
        ...
        """
        Convert image to base64 encoding, handling both local files and URLs.

        Args:
            image_path (str): Either a local path (ws-*.jpg/ms-*.jpg) or URL
            default_image_path (str): Path to default image if primary image fails
        ...
        try:
            ...
            if image_path.startswith(("http://", "https://")):
                ...
                if image_path.startswith(("ws-", "ms-")):
                    full_path = os.path.join("static", "imgs", "students", image_path)
                else:
            ...
            try:
                # Use default image if primary image fails
    ...
    def _create_student_achievements_plots(self, grades_df: pd.DataFrame):
        ...
        # Plot 1: Bar Chart
        ax1 = plt.subplot(221)
        subjects = sorted(grades_df["SubjectName"].unique())
    ...
    def generate_student_profile_report(self, student_id):
        ...
        metadata = {
            ...
            "student": {
                "id": student_id,
                "photo_url": self._get_image_as_base64(student_details.ImageURL),
    ...
    def generate_academic_performance_report(self):
        ...
        grade_ranges = [(0, 49), (50, 59), (60, 69), (70, 79), (80, 89), (90, 100)]
        grade_labels = [
            "F (0 - 49)",
            "E (50 - 59)",
            "D (60 - 69)",
        ...
        colors = [
            "red" if (low == 0 and high == 49) else "skyblue"
            for low, high in grade_ranges
        ]
        bars = ax1.bar(grade_labels, grade_counts, color=colors, edgecolor="black")
        ...
        # passing students >=45
        passing_students = (
            all_grades_df.groupby("StudentID")["percentage"]
            .apply(lambda mark: all(mark >= 50))
            .reset_index()
        )
        passing_students_len = len(
            passing_students[passing_students["percentage"] == True]
        )

        # the number of students got >=90
        top_performers = (
            all_grades_df.groupby("StudentID")["percentage"]
            .apply(lambda mark: all(mark >= 90))
            .reset_index()
        )
        top_performers_len = len(top_performers[top_performers["percentage"] == True])
        ...
        for student_id in top_3_ids:
            ...
            image_base64 = self._get_image_as_base64(
                ...
                default_image_path="static/default_student.png",
            )
        ...
        template_data = {
            ...
            "statistics": {
                ...
                "passing_students": [
                    int(passing_students_len),
                    f"{(passing_students_len/total_students*100):.1f}",
                ],
                "top_performers": [
                    int(top_performers_len),
                    f"{(top_performers_len/total_students*100):.1f}",
                ],
                "top_3_students": top_3_students,
                "departments": [
                    {"Department": dept, "percentage": avg}
                    for dept, avg in dept_avg.items()
                ],
                "subjects": [
                    {"SubjectName": subj, "percentage": avg}
