...
def db_manager():
    ...
    with manager.get_connection() as connection:
        ...
        cursor.execute(
            """CREATE TABLE IF NOT EXISTS Students (StudentID INTEGER PRIMARY KEY, FirstName TEXT, LastName TEXT, UniversityID INTEGER, Email TEXT, DateOfBirth Date, AcademicYear INTEGER, ImageURL TEXT)"""
        )
        ...
        cursor.execute(
            """INSERT INTO Students (StudentID, FirstName, LastName, UniversityID) VALUES (1,"Test","Student",1)"""
        )
...
def test_get_subjects_per_student(db_manager):
    subjects: pd.DataFrame = db_manager.get_subjects_per_student(1).sort_index(axis=1)
...
def test_get_all_students(db_manager):
    all_students: pd.DataFrame = db_manager.get_all_students().sort_index(axis=1)
    # (StudentID, FirstName, LastName, UniversityID) VALUES (1,"Test","Student",1)"""
    # (StudentID INTEGER PRIMARY KEY, FirstName TEXT, LastName TEXT, Email TEXT, DateOfBirth Date, AcademicYear INTEGER, ImageURL TEXT, UniversityID INTEGER)
    expected = pd.DataFrame(
        {
            "StudentID": [1],
            "FirstName": ["Test"],
